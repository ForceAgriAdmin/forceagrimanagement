...
export class FacialRekognitionService {
  private readonly apiBase = 'https://axsvj3vxv1.execute-api.eu-west-1.amazonaws.com/Dev';
  private readonly authBucket = 'workerauthbucket';
  private readonly existingWorkerBucket = 'workerrekognitionbucket';

  constructor(private http: HttpClient) {}

  private uploadToBucket(file: File | Blob, filename: string): Observable<boolean> {
  ...
  return this.http.put(uploadUrl, file, {
    ...
    switchMap(response => {
      if (response.status === 200) {
        ...
        return throwError(() => new Error(`Upload failed with status ${response.status}`));
      }
    })
  );
}
  private runFacialRecognition(filename: string): Observable<awsRekognitionResponse> {
    ...
    return this.http.get<awsRekognitionResponse>(requestFacialUrl, { headers, params });
  }

  PingWorkerFacialRekgnition(file: File | Blob, filename: string): Observable<awsRekognitionResponse> {
  return this.uploadToBucket(file, filename).pipe(
    switchMap(success => {
      if (success) {
